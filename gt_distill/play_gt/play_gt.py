from h5py import File as open_h5
import cv2
import numpy as np
...
FCAM_INTRINSIC_MATRIX = [2648.0, 0.0, 1928.0 / 2,
                                           0.0, 2648.0, 1208.0 / 2,
                                           0.0, 0.0, 1.0]
...
def calib_frame_to_full_frame(in_x, in_y, in_z):
  # pt = [[in_x, in_y, in_z]]
  # Ep = matvecmul3(10, pt)
...
def get_polygon_points(points, idx):
    ...
    print(f"points:{str(points[idx,0,0,-1,:3])}")
    points = points[idx,0,0,:,:3]
    for point in points:                                              # add the points of polygon
        y, x, z = point
        left_calibrated = calib_frame_to_full_frame(x,y, z)
        right_calibrated = calib_frame_to_full_frame(x,y, z)
        left_points.append(left_calibrated)
        right_points.append(right_calibrated)
    polygon_points = left_points + right_points
    return polygon_points

def fill_xyzt(builder, x, x_std=None, y_std=None, z_std=None):
   print(f"x:{str(x)}")
...
class VideoWidget(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.cap = cv2.VideoCapture("train/datasets/Chunk_1/b0c9d2329ad1606b|2018-07-27--06-50-48/8/video.hevc")

        self.timer = QTimer(self)
        self.timer.timeout.connect(self.update_frame)
        self.timer.start(30)  # Update every 30 milliseconds

        self.rect_position = QRect(100, 100, 200, 150)
        gt_file = open_h5("train/datasets/Chunk_1/b0c9d2329ad1606b|2018-07-27--06-50-48/8/gt_distill.h5", "r")
        self.plans = gt_file["plans"]
        self.current_idx = 0

    def update_frame(self):
        ret, frame = self.cap.read()
        if ret:
            self.image = self.convert_cv_to_qimage(frame)
    ...
    def paintEvent(self, event):
        ...
        self.polygon_points = get_polygon_points(self.plans, self.current_idx)
        self.current_idx += 1 
        polygon = QtGui.QPolygonF([QtCore.QPointF(*point) for point in self.polygon_points])
        ...
        # Draw a curved line through specified points using cubic Bézier splines
        for i in range(1, len(self.points) - 10, 2):
            if i + 2 < len(self.points):
                # Use the current point, the next point as the control point,
                # and the point after the next point as the destination point
                path.cubicTo(*self.points[i], *self.points[i + 1], *self.points[i + 2])
            else:
                # If there are fewer than 3 points remaining, use a quadratic Bézier spline
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.central_widget = VideoWidget()
...
if __name__ == '__main__':
    ...
    #     # print(f"vd:{str(frame)}")
    #     # cv2.imshow('First Frame', frame)
    #     # cv2.waitKey(0)  # Wait until a key is pressed
    #     # cv2.destroyAllWindows()
    #     widget.set_polygon(plans)
    # widget.set_polygon([[8,150],[10,16],[20,32], [18,150],[10,16],[20,16]])
    window.setGeometry(100, 100, 800, 600)
    window.setWindowTitle('OpenCV and Qt Overlay')
    window.show()
    sys.exit(app.exec_())
...
void AnnotatedCameraWidget::updateState(const UIState &s) {
  ...
  brake = QString::number(std::nearbyint(cs.getUpAccelCmd()));
...
void AnnotatedCameraWidget::drawHud(QPainter &p) {
  ...
  // brake indicator
  drawText(p, rect().center().x() + 200, 290, brake, 200);
